  ...
  return (
    <div className="w-full h-screen sticky top-0 bg-transparent overflow-hidden">
      <Canvas camera={{ position: [18, 14, 18], fov: 45 }} gl={{ alpha: true }}>
        ...
        <OrbitControls enableZoom={false} enablePan={false} enableRotate={true} maxPolarAngle={Math.PI / 2 - 0.05} />
      </Canvas>
      ...
      <button 
        ...
        className="absolute bottom-8 right-8 z-20 text-xs uppercase tracking-widest text-gray-400 hover:text-white transition-colors border border-gray-800 hover:border-gray-600 rounded-full px-4 py-2 bg-black/50 backdrop-blur-sm pointer-events-auto"
      >
        Skip Animation
      </button>
